...
- [x] Remove `better-auth`, `@better-auth/cli`, `mysql2` from `package.json`
...
- [x] Create `lib/supabase/client.ts` - browser client factory
- [x] Create `lib/supabase/server.ts` - server client factory
- [x] Create `lib/supabase/middleware.ts` - middleware client factory
- [x] **Update `middleware.ts`** - add Supabase session refresh, update matcher to run on all routes
- [x] Rewrite `services/auth.ts` with Supabase client and `useSession` hook

### Phase 3: Auth Routes & Callbacks
- [x] Create `app/api/auth/confirm/route.ts` for magic link verification
- [x] Delete `app/api/auth/[...betterauth]/route.tsx`
- [x] Update `app/auth/callback/page.tsx` - use Supabase session, remove `X-Auth-Provider-Id` header
- [x] Update `app/login/page.tsx` - use `signInWithOtp`

### Phase 4: Protected API Routes Migration
- [x] Update `app/api/users/me/list/route.ts`
- [x] Update `app/api/users/me/list/[uuid]/route.ts`
- [x] Update `app/api/users/me/requests/route.ts`
- [x] Update `app/api/users/me/requests/[arxivId]/route.ts`
- [x] Update `app/api/users/me/papers/[uuid]/processing_metrics/route.ts`
- [x] Delete `app/api/users/sync/route.ts`

### Phase 5: Client Components & Services Migration
- [x] Update `services/users.ts` - remove `authProviderId` param and `buildAuthHeaders()`
- [x] Update `services/requests.ts` - remove `authProviderId` param and `buildAuthHeaders()`
- [x] Update `components/RequireAuth.tsx`
- [x] Update `components/NavBar.tsx`
- [x] Update `components/UserSidebar.tsx`
- [x] Update `components/AddToListButton.tsx` - also remove `sendAddToListMagicLink` usage
- [x] Update `components/AddToListButtonMobile.tsx` - also remove `sendAddToListMagicLink` usage
- [x] Update `components/RequestPaperButton.tsx` - also remove `sendRequestPaperMagicLink` usage
- [x] Update `app/user/layout.tsx`
- [x] Update `app/user/list/page.tsx`
- [x] Update `app/user/requests/page.tsx`
...
### Phase 7: Cleanup
- [x] Delete `lib/server_auth.ts`
- [x] Delete `lib/auth.ts`
- [x] Delete `lib/magicLink.ts`
- [x] Delete `lib/emails/` directory (3 files)
- [x] Remove any remaining `mysql2` imports or connection code
- [x] Remove `resend` dependency (only used in deleted `lib/server_auth.ts`)
- [x] Remove `@react-email/components` and `@react-email/render` dependencies
